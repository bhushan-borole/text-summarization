...
class Extractive_Summarizer():
	...
	def summarize(self, article, line_count=5):
		sentences, cleaned_text = self.clean_and_tokenize(article)
		original_length = len(sentences)
		if original_length > line_count:
			word_frequencies = self.find_word_frequencies(cleaned_text)
			sentence_scores = self.calculate_sentence_scores(sentences, word_frequencies)
			summary = ''.join(sorted(sentence_scores, reverse=True)[:line_count])
			return summary
		else:
